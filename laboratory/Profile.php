                    ...
                    <div class="my-4 pt-2  text-center text-gray-600 dark:text-white">
                        ...
                        <div class="mb-2 flex justify-center mt-4">
                            <div class="relative flex flex-col items-center justify-center" x-data="{ open: false }">
                                <div class="">
                                    <button @click="open = true" class="bg-gray-300 dark:bg-slate-600 border hover:bg-gray-200 py-2 px-4 rounded-md shadow-md transition duration-300 ease-in-out">Update
                                        Profile</button>
                                </div>
                                <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-description="Background backdrop, show/hide based on modal state." class="fixed inset-0 flex items-center justify-center bg-gray-600 bg-opacity-75 transition-opacity" @keydown.escape.window="open = false">
                                    <div class="relative transform overflow-hidden rounded-lg bg-white dark:bg-slate-700 text-left shadow-xl transition-all sm:w-full sm:max-w-lg" @click.away="open = false">
                                        <div class="sm:p-10">
                                            ...
                                            <form id="updateLabDetailForm" method="post" action="../Backend/CRUD.php">
                                                <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-4 text-black dark:text-white font-mono">
                                                    <div class="md:col-span-4">
                                                        <input type="hidden" name="labid" value="<?php echo $nested_array["lab_id"]; ?>">
                                                        <label class="text-lg" for="fullname">Full Name</label>
                                                        <input type="text" name="fullname" id="fullname" class="h-10 border mt-1 rounded px-4 w-full bg-slate-200 dark:bg-slate-600" value="<?php echo $nested_array["lab_name"]; ?>" />
                                                    </div>

                                                    <div class="md:col-span-2">
                                                        <label class="text-lg" for="email">Email Address</label>
                                                        <input type="email" name="email" id="email" class="h-10 border mt-1 rounded px-4 w-full bg-slate-200 dark:bg-slate-600" value="<?php echo $nested_array["email"]; ?>" />
                                                    </div>

                                                    <div class="md:col-span-2">
                                                        <label class="text-lg" for="contact">Contact</label>
                                                        <input type="text" name="contact" id="contact" class="h-10 border mt-1 rounded px-4 w-full bg-slate-200 dark:bg-slate-600" value="<?php echo $nested_array["contact"]; ?>" />
                                                    </div>

                                                    <div class="md:col-span-4">
                                                        <label class="text-lg" for="address">Address / Street</label>
                                                        <input type="text" name="address" id="address" class="h-10 border mt-1 rounded px-4 w-full bg-slate-200 dark:bg-slate-600" value="<?php echo $nested_array["lab_add"]; ?>" />
                                                    </div>

                                                    <div class="md:col-span-2">
                                                        <label class="text-lg" for="city">City</label>
                                                        <input type="text" name="city" id="city" class="city h-10 border mt-1 rounded px-4 w-full bg-slate-200 dark:bg-slate-600" value="<?php echo $nested_array["city"]; ?>" />
                                                    </div>

                                                    <div class="md:col-span-2">
                                                        <label class="text-lg" for="state">State</label>
                                                        <input type="text" name="state" id="state" class="state h-10 border mt-1 rounded px-4 w-full bg-slate-200 dark:bg-slate-600" value="<?php echo $nested_array["state"]; ?>" />
                                                    </div>

                                                </div>
                                                <div class="mt-6 flex justify-center gap-x-4">
                                                    <button type="sbumit" value="" name="updateLabDetail" class="py-2 w-52 inline-flex justify-center items-center gap-2 rounded-md font-medium bg-gray-300 dark:bg-slate-800 dark:hover:border cursor-pointer shadow-lg align-middle hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-green-700 focus:ring-green-600 transition-all text-lg">Save Update</button>
