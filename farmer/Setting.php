...
<section class="w-full h-full mx-auto my-8 p-8 rounded-md">
    ...
    <div class="h-20 w-full my-8 rounded-md bg-gray-200 dark:bg-slate-700 transition-all duration-300 grid grid-cols-1 sm:grid-cols-3">
        <div onclick="loadUpdateForm('updateProfile','<?php echo $_SESSION['farmer']; ?>')" class="w-[50%] h-full rounded-lg bg-gray-300 dark:bg-slate-600 flex justify-center items-center text-black dark:text-gray-300 text-xl">Update Profile Details</div>
        <div onclick="loadUpdateForm('updatePassword','<?php echo $_SESSION['farmer']; ?>')" class="w-[50%] h-full rounded-lg bg-gray-300 dark:bg-slate-600 flex justify-center items-center text-black dark:text-gray-300 text-xl">Update Password</div>
